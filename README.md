...
- 🔧 **自定义 CSS 选择器**：支持完整的 CSS 选择器语法，可屏蔽没有 class / id 的具体元素（如 `a[target="_blank"][href="#"]`）
...
3. 填写：
   ...
   - **自定义 CSS 选择器（可选）**：如 `a[target="_blank"][href="#"]`，用于屏蔽没有 class / id 的具体元素
4. 点击 **「保存」**
...
| 输入示例                                      | 匹配方式   | 说明                                   |
| --------------------------------------------- | ---------- | -------------------------------------- |
| `a[target="_blank"][href="#"]`                | 属性选择器 | 精确屏蔽带有指定属性的 `<a>` 标签      |
| `div[data-ad] > a[href*="click"]`             | 复合选择器 | 屏蔽特定祖先结构下的元素               |
| `a[target="_blank"][href="#"]` + `.ad-banner` | 多个       | 每行一个或逗号分隔，一次性匹配多个目标 |

> 自定义 CSS 选择器按**完整 CSS 选择器语法**原样使用，适合屏蔽没有 class / id 的具体元素（如全屏覆盖层）。「屏蔽元素」和「自定义 CSS 选择器」可同时填写，取并集生效。
...
- 支持「className / id 快捷输入」与「自定义 CSS 选择器」两种方式，可同时使用
...
A: 使用「自定义 CSS 选择器」功能，例如 `a[target="_blank"][href="#"]` 可以直接按标签属性精确匹配并屏蔽该元素。
